...
public class MessagesRepositoryImpl implements MessagesRepository<Message>{
    ...
    private final RowMapper<Message> messageRowMapper = (resultSet, rowNum) -> {
        ...
        String room_name = resultSet.getString("name");

        message.setRoom(new Chatroom(room_id, room_name, new User(id, username, password)));
        return message;
    };
    ...
    public void save(Message message) {
        ...
        jdbcTemplate.update(factory.newPreparedStatementCreator(Arrays.asList(message.getAuthor().getIdentifier(), message.getRoom().getIdentifier(), message.getText(), message.getDateTime())), keyHolder);
    ...
    public void update(Message message) {
        ...
        jdbcTemplate.update(query, message.getAuthor().getIdentifier(), message.getRoom().getIdentifier(), message.getText(), message.getDateTime(), message.getIdentifier());
    }
    ...
    @Override
    public List<Message> findForChatroom(Chatroom chatroom) {
        String query = "SELECT * FROM (" +
                ...
                "  LIMIT 30" +
                ") AS recent_messages " +
                "ORDER BY recent_messages.message_datetime ASC";
        return jdbcTemplate.query(query, new Object[]{chatroom.getIdentifier()}, messageRowMapper);
    }
